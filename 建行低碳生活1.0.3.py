    ...
    def user_info(self, user_name):
        ...
        try:
            ...
            if level_data is not None:
                ...
                send_msg += f'-用户【{user_name}】等级【{tree_level}】: {island_name}\n'
            else:
                # 失败
                print('-查询失败')
                send_msg += '-查询失败\n'
            ...
            if now.hour < 12:
                ...
                if rcrd_num_data is not None:
                    ...
                    if Rvl_Rcrd_Num == 0:
                        ...
                        send_msg += f'-当前可捡次数: {Rvl_Rcrd_Num}\n'
                    else:
                        print(f'-当前可捡次数: {Rvl_Rcrd_Num},待领取能量值: {Avl_APnt}')
                        send_msg += f'-当前可捡次数: {Rvl_Rcrd_Num},待领取能量值: {Avl_APnt}\n'
                        ...
                        for value in LIST1:
                            ...
                            if receive_data is not None:
                                ...
                                send_msg += f'-领取成功，获得能量: {APnt_Hpn_Num}\n'
                            else:
                                print('-领取失败')
                                send_msg += '-领取失败\n'
                else:
                ...
                if upload_data is not None:
                    ...
                    send_msg += f'-上传步数成功，获得能量: {Apnt_Hpn_Num}\n'
                else:
                    print('-上传步数失败')
                    send_msg += '-上传步数失败\n'
        except Exception as e:
        ...
        try:
            ...
            if list_res is not None:
                ...
                if coupon_title is None:
                    ...
                    send_msg += f'未找到{exchange_name}，请检查是否有该类型的奖品。\n'
                    return
                elif int(APnt_Bal) < int(coupon_price):
                    print('当前碳能量不足，无法兑换~~')
                    send_msg += f'当前碳能量不足，无法兑换~~\n'
                    return
                else:
                    # 开始兑换
                    code_payload = {
                    ...
                    send_msg += f'开始兑换: {coupon_title}, 库存数量: {Remain_Num}\n'
...
if __name__ == "__main__":
    ...
    if not token:
        ...
        send_msg += f'⛔️未获取到ck变量：请检查变量 {env_name}是否填写\n'
        exit(0)
    cookies = re.split(r'[@\n]', token)
    msg = f"低碳生活共获取到{len(cookies)}个账号"
    send_msg += f'低碳生活共获取到{len(cookies)}个账号\n'
    print(msg)

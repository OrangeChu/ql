...
class CCD:
    ...
    def ccbLife(self):
        ...
        if IS_SIGN == 1:
            ...
            if signin_data.get('errCode', '') == '0':
                ...
                send_msg += f'-签到成功,已签到天数: {SIGN_DAY}\n'
                self.reward_day(SIGN_DAY, REWORD_LIST, REWARD_NODES)
            else:
                print(signin_data)
        ...
        if REWORD_LIST:
            for reward in REWORD_LIST:
                ...
                if DISTRIBUTE_STATUS == 1:
                    ...
                    send_msg += f'-已领取奖励: {COUP_TITLE} {COUP_SUB_TITLE}\n'
                else:
                    print(f'-领取签到{SIGN_DAY}天奖励')
                    send_msg += f'-领取签到{SIGN_DAY}天奖励\n'
    ...
    def receive(self, SIGN_DAY, coupon_id, coupon_type, dccp):
        ...
        if receive_data['errCode'] != '0':
            ...
            send_msg += f'-{receive_data.get("data").get("couponName")}: 领取成功\n'
